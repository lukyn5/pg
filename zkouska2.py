...
def convert_to_czk(amount, currency):
    ...
    try:
        ...
        response.raise_for_status() # Ověří, zda stahování proběhlo v pořádku
        lines = response.text.strip().splitlines()
    except requests.RequestException as e:
        raise RuntimeError(f"Nepodařilo se stáhnout kurzovní lístek: {e}")

    # První dva řádky jsou datum a hlavička, data začínají od 3. řádku
    for line in lines[2:]:
        parts = line.split('|')
        ...
        if code == currency:
            # Načtení množství (např. 1, 100, 1000)
            unit = float(parts[2])
            
            # Načtení kurzu - pozor na desetinnou čárku, kterou Python vyžaduje jako tečku
            rate_str = parts[4]
            rate = float(rate_str.replace(',', '.'))
            
            # Výpočet: (částka * kurz) / množství
            # Dělení množstvím je nutné pro měny jako HUF, JPY atd., kde je kurz udáván za 100 nebo 1000 jednotek
            result = (amount * rate) / unit
            
            return round(result, 2)

    # Pokud cyklus doběhne a měna nebyla nalezena
